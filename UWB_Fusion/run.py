...
class UWBFusion:
    ...
    def __init__(self, N: int, DIM: int,
                 pose_topic: str, dist_topic: str, pub_pose_topic: str, pub_traj_topic: str,
                 iteration=8, buffer=16):
        ...
        self.pose_publisher = rospy.Publisher(pub_pose_topic, PoseArray, queue_size=10)
        self.traj_publisher = {
            i: rospy.Publisher(
                pub_traj_topic + str(i), Path, queue_size=10
            ) for i in range(self.N)
        }
        threading.Thread(target=self.pub_trajectory).start()
        threading.Thread(target=self.pub_pose).start()
        self.open_rviz()
    ...
    @staticmethod
    def open_rviz():
        rospy.loginfo('Waiting for opening and configuration of rivz...')
        command = ['rviz', '-d', 'traj_vis.rviz']
        ...
        while not rospy.is_shutdown():
            if len(list(self.est)) != 0:
                ...
                for i in range(self.N):
                    ...
                    # self.ax.plot(gt[:, i, 0], gt[:, i, 1], linestyle='--', color='gray', linewidth=2)
            ...
            if len(list(self.est)) != 0:
                ...
                for points in est:
                    for i, point in enumerate(points):
                        ...
                        paths[i].poses.append(pose)

                [self.traj_publisher[i].publish(paths[i]) for i in range(self.N)]
            rate.sleep()
    ...
    def __call__(self):
        rospy.loginfo('Waiting for UWB distances matrix...')
        while not rospy.is_shutdown():
            if self.pose_ready and self.dist_ready:
                ...
                rospy.loginfo(f'Total time cost: {time.time() - t_start} ms.')
...
def get_param():
    ...
    parser.add_argument('--pub_pose_topic', type=str, default='/uwb1/uwb_locator/pose')
    parser.add_argument('--pub_traj_topic', type=str, default='/uwb1/uwb_locator/traj')
...
def run():
    ...
    uwb_system = UWBFusion(args.N, args.DIM,
                           args.pose_topic, args.dist_topic, args.pub_pose_topic, args.pub_traj_topic,
                           args.iteration)
    uwb_system.left_fixed_point = 0
    uwb_system.right_fixed_point = 0

    threading.Thread(target=uwb_system).start()
    # Must in main thresh
    uwb_system.realtime_visualization()
